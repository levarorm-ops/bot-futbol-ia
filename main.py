...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bienvenida = (
        ...
        "Escríbeme con total libertad lo que quieres analizar.\n"
        "Ejemplo: *'Analízame el partido de Real Madrid vs Barcelona de la liga española'* "
        "o pásame los goles esperados directamente (ej: `1.8 1.1`)."
    )
...
async def procesar_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Si el usuario escribe números directamente, los usamos como goles esperados
    palabras = update.message.text.split()
    l_local, l_visitante = 1.6, 1.1 # Valores predeterminados inteligentes si habla en texto natural

    try:
        if len(palabras) >= 2:
            # Intentar extraer números si los puso en el texto
    ...
    await update.message.reply_text(
        f"🧠 Entendido. Analizando tu solicitud sobre: *'{update.message.text}'*...\n"
...
def main():
    ...
    # Captura cualquier mensaje de texto normal que escribas en el chat de forma inteligente
    app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), procesar_mensaje))

    print("¡Bot inteligente con lenguaje natural activo!")
