...
# Permitir bucles de eventos anidados en hilos secundarios
nest_asyncio.apply()
...
# ==========================================
def evaluar_perfil_localia(
    lambda_local: float, factor_fortaleza: float
) -> dict:
    ifl_score = round(
        min(98.0, max(40.0, (lambda_local * 42.0) * factor_fortaleza)), 1
    )
    ...
    if es_fortaleza_alta:
        diagnostico_local = "🔥 **FORTALEZA LOCAL ALTA:** Equipo highly fiable en casa para sostener la Doble Oportunidad (1X)."
...
def evaluar_intensidad_temprana(lambda_local: float) -> dict:
    ...
    return {
        ...
        "mercado_sugerido": (
            "Local Gol en 1º Tiempo / Más de 0.5 Goles 1HT"
            if es_equipo_intenso
            else "Partido de desgaste progresivo / Control táctico"
        ),
    }


def simular_poisson_avanzado(
    lambda_l: float, lambda_v: float, simulaciones: int = 100000
):
    ...
    marcadores, conteos = np.unique(
        list(zip(goles_local, goles_visita)), axis=0, return_counts=True
    )
    ...
    return {
        ...
        "marcador_exacto": (
            f"{marcador_mas_probable[0]} - {marcador_mas_probable[1]}"
        ),
...
def calcular_kelly_fraccional(
    probabilidad: float, cuota: float, bankroll: float = 1000000.0
) -> dict:
...
async def generar_reporte_contextual(partido: str) -> dict:
    ...
    return {
        "resumen_narrativo": (
            f"📋 **EVALUACIÓN PREDICTIVA Y COMPORTAMIENTO DE LOCALÍA"
            f" ({partido}):**\n• **Auditoría de Dominio en Casa:** Evaluando"
            " si el cuadro local es un equipo dominante (tendencia 1X) o un"
            " local apático de juego friccionado.\n• **Filtro de Alta"
            " Seguridad (85% - 95%+):** Mercados seleccionados tras 100,000"
            " iteraciones."
        ),
        "arbitro_y_h2h": (
            "⚖️ **Perfil Arbitral & Faltas:** Promedio de 4.6 tarjetas."
            " Tendencia a interrupciones si el local despliega un juego de"
            " posesión pasiva.\n⚔️ **Histórico H2H:** Monitoreo de constancia en"
...
async def enviar_menu_principal(update_or_query, contexto_es_callback=False):
    texto_menu = (
        ...
        "Gestionando tu bankroll de **1,000,000 COP** con radar de **Fortaleza"
        " de Localía y Alta Seguridad**:\n"
        "🏰 **Módulo Gana/Empata (1X):** Identificación automática de locales"
        " dominantes vs. locales apáticos.\n"
        "🎯 **Filtro Exclusivo:** Selección de mercados con **85% a 95%+** de"
        " probabilidad real.\n"
        "⚽ **Ligas Foco:** Austria, Col (A), Bra, Arg, Ing, Ale, Esp, Ita, Fra,"
        " Por, Ara, Rus, Tur, Din, Sue, Pol, Cro, Chi, Esc, Bul.\n\n"
    ...
    teclado = [
        [
            InlineKeyboardButton(
                "🌍 Ligas Priorizadas & Cobertura", callback_data="ver_todas"
            )
        ],
        [
            InlineKeyboardButton(
                "💰 Gestión de Bankroll (1M COP)", callback_data="info_millon"
            )
        ],
    ]
    ...
    if contexto_es_callback:
        await update_or_query.message.edit_text(
            texto_menu, reply_markup=reply_markup, parse_mode="Markdown"
        )
    else:
...
async def manejar_botones(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ver_todas":
        texto = (
            ...
            "• **América:** Colombia (Solo Primera A), Brasil (Serie A y Copa),"
            " Argentina.\n"
            "• **Europa Top & Copas:** Austria (Bundesliga y Copa), Alemania"
            " (B1, B2, DFB-Pokal), Inglaterra (PL, Championship, FA Cup, EFL"
            " Cup), España, Italia, Francia, Portugal.\n"
            "• **Ligas de Alto Volumen:** Arabia Saudí, Rusia, Turquía,"
            " Escocia, Dinamarca, Suecia, Polonia, Croacia, Bulgaria, China."
        )
        teclado = [
            [InlineKeyboardButton("⬅️ Volver al Menú", callback_data="ir_menu")]
        ]
        await query.message.edit_text(
            texto, reply_markup=InlineKeyboardMarkup(teclado), parse_mode="Markdown"
        )
    elif data == "info_millon":
        texto = (
            "💎 **Estrategia del Millón (1M COP):**\nEl sistema audita el Valor"
            " Esperado (EV) y sugiere únicamente entradas de alta probabilidad"
            " (85%-95%+) con límite institucional del 5% del capital."
        )
        teclado = [
            [InlineKeyboardButton("⬅️ Volver al Menú", callback_data="ir_menu")]
        ]
...
async def procesar_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔍 **Analizando Localía y Proyectando Probabilidades:**"
        f" *'{texto_usuario}'*...\n*(Auditando perfil de juego en casa,"
        " tendencia 1X y filtro 85%-95%+)*",
        parse_mode="Markdown",
    )
    ...
    mercados_alta_seguridad = [
        m for m in todos_los_mercados if m[1] >= 85.0
    ]
    mercados_alta_seguridad = sorted(
        mercados_alta_seguridad, key=lambda x: x[1], reverse=True
    )

    if mercados_alta_seguridad:
        top_mercados_str = "\n".join(
            [
                f"• **{m[0]}:** `{m[1]:.1f}%` de probabilidad real"
                for m in mercados_alta_seguridad
            ]
        )
    else:
        mercados_ordenados_fall = sorted(
            todos_los_mercados, key=lambda x: x[1], reverse=True
        )[:3]
        top_mercados_str = (
            "⚠️ *Ningún mercado alcanzó el umbral del 85%. Opciones con mayor"
            " probabilidad:*\n"
            + "\n".join(
                [
                    f"• **{m[0]}:** `{m[1]:.1f}%`"
                    for m in mercados_ordenados_fall
                ]
            )
        )
    ...
    if cuota_betano_usuario:
        ...
        if ev_val > 0:
            evaluacion_ev = (
                f"✅ **FILTRO EV BETANO:** `¡Valor Positivo!"
                f" (+{ev_val:.1f}%)`. La cuota ofrecida"
                f" (@{cuota_betano_usuario}) cubre el riesgo y supera la"
                f" cuota justa (@{res['cuota_justa_local']})."
            )
        else:
            evaluacion_ev = (
                f"❌ **FILTRO EV BETANO:** `EV Negativo ({ev_val:.1f}%)`."
                f" Betano paga @{cuota_betano_usuario}, pero según la"
                f" simulación la cuota mínima justa es"
                f" @{res['cuota_justa_local']}. **Descartar**."
            )
    else:
        evaluacion_ev = (
            "💡 **Sugerencia:** Agrega la cuota que paga Betano (ej:"
            f" `{texto_usuario} @ 2.05`) para calcular el EV exacto."
        )

    cuota_evaluar = (
        cuota_betano_usuario
        if cuota_betano_usuario
        else res["cuota_justa_local"]
    )
    ...
    respuesta = (
        ...
        f"• **Marcador Exacto Más Probable:** `{res['marcador_exacto']}`"
        f" (Probabilidad: `{res['prob_marcador']}%`)\n\n"
        f"🎯 **RECOMENDACIÓN DE MERCADOS TOP (85% A 95%+ PROBABILIDAD):**\n"
        f"{top_mercados_str}\n\n"
        f"🚩 **Proyección de Córners (Ajustado por Inercia):**\n"
        f"• Local: ~`{res['corners_local']}` | Visitante:"
        f" ~`{res['corners_visita']}` | Total: ~`{res['total_corners']}`"
        f" córners\n\n"
        f"{contexto['arbitro_y_h2h']}\n\n"
        f"⚽ **Simulación Estocástica (100,000 Iteraciones):**\n"
        f"• Victoria Local: `{res['p_local']:.1f}%` (Cuota Justa:"
        f" `{res['cuota_justa_local']}`)\n"
        f"• Empate: `{res['p_empate']:.1f}%` | Victoria Visitante:"
        f" `{res['p_visitante']:.1f}%`\n"
        f"• Más de 2.5 Goles: `{res['p_over_2_5']:.1f}%` (Cuota Justa:"
        f" `{res['cuota_justa_over']}`)\n\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 **AUDITORÍA DE VALOR Y CUOTAS:**\n"
        f"{evaluacion_ev}\n\n"
        f"💰 **GESTIÓN DE STAKE (Bankroll 1M COP):**\n"
        f"• Inversión Sugerida: `${kelly['stake']:,.0f} COP`"
        f" (`{kelly['porcentaje']:.1f}%` de tu banca)\n"
...
# ==========================================
# 5. EJECUCIÓN DEL BOT EN SEGUNDO PLANO
# ==========================================
def arrancar_bot_telegram(token):
...
# ==========================================
# 6. INTERFAZ Y PANEL DE CONTROL STREAMLIT
# ==========================================
st.set_page_config(
    page_title="BETANO Analytics Panel", page_icon="🤖", layout="wide"
)

st.title("🤖 Panel de Control - BETANO Analytics Bot")
st.write(
    "Estado del bot de Telegram en segundo plano e historial de análisis."
)

TELEGRAM_TOKEN = st.secrets.get("TELEGRAM_TOKEN") or os.getenv(
    "TELEGRAM_BOT_TOKEN"
)

if not TELEGRAM_TOKEN:
    st.error(
        "❌ No se encontró el token de Telegram. Por favor agrégalo en los"
        " Secrets de Streamlit como TELEGRAM_TOKEN."
    )
else:
    if "bot_iniciado" not in st.session_state:
        st.session_state["bot_iniciado"] = True
        hilo = threading.Thread(
            target=arrancar_bot_telegram,
            args=(TELEGRAM_TOKEN,),
            daemon=True,
        )
        hilo.start()
        st.success("🚀 Bot iniciado correctamente en segundo plano.")
    else:
        st.info("🟢 El bot ya se encuentra activo y escuchando en Telegram.")
...
if os.path.exists(DB_PATH):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT partido, cuota_betano, cuota_justa, prob_local, ev_porcentaje,"
        " stake_cop, timestamp FROM historial_analisis ORDER BY id DESC"
    )
    ...
    if filas:
        st.dataframe(
            ...
            column_config={
                ...
                "1": "Cuota Betano",
                "2": "Cuota Justa",
                "3": "Probabilidad Local (%)",
                "4": "EV (%)",
                "5": "Stake Sugerido (COP)",
                "6": "Fecha y Hora",
            },
        ...
        st.write(
            "Aún no hay análisis registrados. Envía un mensaje a tu bot en"
            " Telegram para comenzar."
        )
else:
    st.write("Base de datos en proceso de inicialización...")
