...
# ----------------------------------------------------
# 2. MOTOR ÚNICO: GROQ (CON DEPORTACIÓN DE ERRORES)
# ----------------------------------------------------
PROMPT_SISTEMA = """
...
def generar_analisis_ia(prompt):
    ...
    try:
        ...
        print("🚀 Enviando petición HTTP a Groq...")
...
if bot:
    ...
    def responder_consulta(message):
        ...
        if not respuesta:
            bot.send_message(message.chat.id, "⚠️ **Groq no pudo procesar la consulta.** Revisa los Logs en vivo de Render para ver el código de error exacto.")
            return
...
@app.route('/')
def home():
    return "Bot Deportivo Operativo con Groq."
