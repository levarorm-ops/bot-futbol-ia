...
def generar_analisis_ia(prompt):
    ...
    if OPENROUTER_KEY:
        try:
            print("[IA Engine] Intentando consultar con OpenRouter...")
            respuesta = consultar_openrouter(prompt)
            print("[IA Engine] ✅ Respuesta obtenida exitosamente de OpenRouter.")
            return respuesta
        except Exception as e:
            print(f"[IA Engine] ❌ ERROR en OpenRouter: {repr(e)}")
    else:
        print("[IA Engine] ⚠️ OPENROUTER_API_KEY no está configurada.")

    if GROQ_KEY:
        try:
            print("[IA Engine] Intentando consultar con Groq...")
            respuesta = consultar_groq(prompt)
            print("[IA Engine] ✅ Respuesta obtenida exitosamente de Groq.")
            return respuesta
        except Exception as e:
            print(f"[IA Engine] ❌ ERROR en Groq: {repr(e)}")
    else:
        print("[IA Engine] ⚠️ GROQ_API_KEY no está configurada.")

    if GEMINI_KEY:
        try:
            print("[IA Engine] Intentando consultar con Gemini...")
            respuesta = consultar_gemini(prompt)
            print("[IA Engine] ✅ Respuesta obtenida exitosamente de Gemini.")
            return respuesta
        except Exception as e:
            print(f"[IA Engine] ❌ ERROR en Gemini: {repr(e)}")
    else:
        print("[IA Engine] ⚠️ GEMINI_API_KEY no está configurada.")

    print("[IA Engine] ⚠️ TODAS LAS IAs FALLARON. Revisa las claves de API en Render.")
