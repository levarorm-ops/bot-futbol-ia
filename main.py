...
print(f"🔑 [INICIO] Groq: {bool(GROQ_KEY)} | Gemini: {bool(GEMINI_KEY)} | OpenRouter: {bool(OPENROUTER_KEY)}")

# ----------------------------------------------------
# 2. BÚSQUEDA WEB AUTÓNOMA (DUCKDUCKGO)
# ----------------------------------------------------
def buscar_informacion_partido(partido):
    print(f"🔍 [SCRAPER] Buscando noticias y alineaciones para: '{partido}'")
    ...
    try:
        ...
        if results:
            ...
            print("✅ [SCRAPER] Información en tiempo real recopilada.")
        else:
...
# ----------------------------------------------------
# 3. PROMPTS DE LOS 11 PUNTOS (SUBDIVIDIDOS)
# ----------------------------------------------------
PROMPT_BLOQUE_1 = """
...
Escribe ÚNICAMENTE los puntos del 1 al 6 de forma clara, concisa y detallada:
...
# ----------------------------------------------------
# 4. MOTORES DE INTELIGENCIA ARTIFICIAL (FALLBACKS)
# ----------------------------------------------------
def consultar_gemini(prompt_sistema, prompt_usuario):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            print("✅ [IA] Respuesta obtenida exitosamente vía Gemini.")
...
def consultar_groq(prompt_sistema, prompt_usuario, modelo="llama-3.1-8b-instant"):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            res = json.loads(response.read().decode("utf-8"))
            print(f"✅ [IA] Respuesta obtenida exitosamente vía Groq ({modelo}).")
...
def consultar_openrouter(prompt_sistema, prompt_usuario):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            res = json.loads(response.read().decode("utf-8"))
            print("✅ [IA] Respuesta obtenida exitosamente vía OpenRouter.")
            return res["choices"][0]["message"]["content"]
    except Exception as e:
...
if bot:
    ...
    def responder_consulta(message):
        ...
        if not parte_1 and not parte_2:
            bot.send_message(message.chat.id, "⚠️ No fue posible conectar con los servidores de IA en este momento. Inténtalo nuevamente en unos segundos.")
            return
