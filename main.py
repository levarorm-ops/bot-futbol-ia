...
def iniciar_telegram_polling():
    if not bot:
        return
    print("[Telegram] Esperando 5 segundos para asegurar limpieza de conexiones previas...")
    time.sleep(5)  # Retraso preventivo contra el error 409
    while True:
        try:
            bot.remove_webhook()
            print("[Telegram] Iniciando polling activo...")
            bot.infinity_polling(timeout=20, long_polling_timeout=10, skip_pending=True)
        except Exception as e:
            print(f"[Polling Error] Conflicto o desconexión detectada: {e}. Reintentando en 5s...")
            time.sleep(5)
