...
DB_PERFILES_TACTICOS = {
    ...
    "default": {
        ...
        "friccion_nivel": "Estándar de la competición",
        "corner_base": 5.0,
        "p_perd": 1.30,
        "p_gan": 0.80,
        "descanso": "72-96 horas",
        "fatiga": "Estándar",
        "distraccion": "📅 Calendario regular.",
        "novedades": "Plantilla normal.",
        "racha": "📊 **RACHA:** Variable.",
        "fortaleza": "🔥 **FORTALEZA:** Competitivo.",
        "debilidad": "⚠️ **DEBILIDAD:** Ajustes pendientes."
    }
}
...
# ----------------------------------------------------
# 4. MOTOR MATEMÁTICO Y PARSER FLEXIBLE (1X2)
# ----------------------------------------------------
...
def procesar_entrada_usuario(texto_usuario):
    ...
    partido_texto = "Partido"
    if len(lineas) > 0:
        partido_texto = lineas[0]
        for num in cuotas_encontradas:
            partido_texto = partido_texto.replace(str(num), '').replace(str(num).replace('.', ','), '')
        partido_texto = partido_texto.strip()

    partido_real, liga_detectada = buscar_partido_real_en_web(partido_texto if len(partido_texto) > 1 else "Partido")
...
def simular_partido_con_cuotas(partido_input, liga_detectada, c_local, c_empate, c_visitante):
    ...
    # Auditoría avanzada si el usuario ingresó las 3 cuotas (Local, Empate, Visitante)
    auditoria_1x2 = ""
    if c_local and c_empate and c_visitante:
        # Calcular probabilidades implícitas de la casa de apuestas sacando el 100% / cuota
...
• Análisis: Cuota de Empate detectada (`{c_empate}`). Ideal para coberturas o doble oportunidad con valor.
"""
    elif c_local:
        cuota_justa_local = round(100 / prob_win_local, 2)
        ev_positivo = c_local > cuota_justa_local
        estado_ev = "✅ ¡Valor Positivo (+EV)!" if ev_positivo else "⚠️ Cuota ajustada."
        auditoria_1x2 = f"""
━━━━━━━━━━━━━━━━━━━
🎯 **AUDITORÍA DE VALOR:**
• Cuota Local: `{c_local}` | Justa: `{cuota_justa_local}` -> {estado_ev}
"""
...
if bot:
    ...
    def cmd_start(message):
        enviar_telegram_seguro(
            message.chat.id,
            "👋 **¡Mind Pay Activo!**\n\nEnvía tu partido y las cuotas en varias líneas (Local, Empate, Visitante) para auditar el mercado."
        )
    ...
    def responder_consulta(message):
        ...
        if texto_usuario.lower() in ["hola", "buenas", "saludos", "hey"]:
            enviar_telegram_seguro(message.chat.id, "👋 ¡Hola! Envíame el partido y tus cuotas organizadas por líneas.")
            return

        enviar_telegram_seguro(message.chat.id, f"🔍 *Rastreando encuentro oficial y auditando cuotas 1X2...*")
