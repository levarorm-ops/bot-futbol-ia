...
if bot:
    ...
    def cmd_start(message):
        print(f"📩 [COMANDO] /start recibido del usuario {message.chat.id}")
    ...
    def responder_consulta(message):
        print(f"📩 [MENSAJE RECIBIDO] Texto: '{message.text}' de {message.chat.id}")
        ...
        if texto_usuario in saludos:
            print("👋 [FILTRO] Saludo detectado, respondiendo localmente.")
        ...
        print("⚽ [PROCESO] Pasando mensaje a la cascada de IAs...")
        bot.send_chat_action(message.chat.id, "typing")
        respuesta = generar_analisis_ia(message.text)
        
        if not respuesta:
            print("⚠️ [ADVERTENCIA] La cascada retornó None (ninguna IA respondió).")
        ...
        print("✅ [RESPUESTA] Enviando análisis de 11 puntos a Telegram...")
