...
if bot:
    ...
    def responder_consulta(message):
        ...
        # Filtro inteligente para que los saludos NO disparen la IA
        ...
        # Si es un partido, activamos la IA
        bot.send_chat_action(message.chat.id, "typing")
        respuesta = generar_analisis_ia(message.text)
        
        if not respuesta:
            bot.send_message(message.chat.id, "⚠️ **Error de conexión con las APIs de IA.** Revisa los Logs en Render para ver si alguna clave (OpenRouter/Groq/Gemini) tiene problemas de saldo o permisos.")
            return

        # Envío fraccionado si es muy largo
