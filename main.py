...
def run_monte_carlo(lambda_home=1.5, lambda_away=1.2, sims=100000):
    ...
    # Probabilidad de Menos de 3.5 Goles globales (ideal para partidos cerrados)
    total_goals = home_goals + away_goals
    under_35_goals = np.mean(total_goals < 3.5) * 100
    
    # Mercados Secundarios: Esquinas (Líneas 5.5 a 10.5), Tarjetas y Tiros a Puerta
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Control inteligente de entradas vacías, puntos o saludos iniciales
    ...
    # Envío de respuesta (segmentada si supera el límite de Telegram de 4000 caracteres)
    if len(report) > 4000:
...
def main():
    ...
    logger.info("Bot de Telegram iniciado correctamente y listo para operar.")
